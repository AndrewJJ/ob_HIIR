...
template <int Order>
class FftScope final : public juce::Component, public juce::Timer
{
public:
...
private:
    ...
    void paintFft (juce::Graphics& g) const;
    void paintFftScale (juce::Graphics& g) const;
    ...
    juce::String hertzToString (const double frequencyInHz, const int numDecimals, const bool appendHz, const bool includeSpace) const;
    static juce::Colour getColourForChannel (const int channel);
    void preCalculateVariables();

    Background background;
    Foreground foreground;
	FftProcessor<Order>* fftProcessor;
    juce::HeapBlock<float> x, y;
    ...
    typename juce::WeakReference<FftScope<Order>>::Master masterReference;
    friend class juce::WeakReference<FftScope<Order>>;
    ListenerRemovalCallback removeListenerCallback = {};

    juce::Atomic<bool> dataFrameReady;

    // Candidate frequencies for drawing the grid on the background
    juce::Array<float> gridFrequencies = { 20.0f, 50.0f, 125.0f, 250.0f, 500.0f, 1000.0f, 2000.0f, 4000.0f, 8000.0f, 16000.0f, 32000.0f, 64000.0f };
...
template <int Order>
void FftScope<Order>::Background::paint (juce::Graphics& g)
{
...
template <int Order>
void FftScope<Order>::Foreground::paint (juce::Graphics& g)
{
        ...
        fftProcessor (nullptr)
{
    ...
    foreground.setMouseCursor (juce::MouseCursor::CrosshairCursor);
...
template <int Order>
void FftScope<Order>::paint (juce::Graphics&)
{ }
...
template <int Order>
void FftScope<Order>::mouseMove (const juce::MouseEvent& event)
{
    currentX = event.x;
    currentY = event.y;
...
template <int Order>
void FftScope<Order>::mouseExit (const juce::MouseEvent&)
{
...
template <int Order>
void FftScope<Order>::prepare (const juce::dsp::ProcessSpec& spec)
{
    samplingFreq = spec.sampleRate;
    preCalculateVariables();
    juce::WeakReference<FftScope<Order>> weakThis = this;
...
template <int Order>
void FftScope<Order>::paintFft (juce::Graphics& g) const
{
    ...
    for (auto ch = 0; ch < fftProcessor->getNumChannels(); ++ch)
    {
        ...
        const auto pst = juce::PathStrokeType (1.0f);
    ...
    if (currentX >= 0 && currentY >= 0)
    {
        ...
        g.setFont (juce::Font (GUI_SIZE_F(0.5)));
        const auto freq = toHzFromPx (static_cast<float> (currentX));
        const auto freqStr = hertzToString (freq, 2, true, true);
        const auto dbStr = juce::String (toDbVFromPx (static_cast<float> (currentY)), 1);
        ...
        if (lblX + lblW > getWidth())
        {
            ...
            lblJust = juce::Justification::centredRight;
        }
...
template <int Order>
void FftScope<Order>::paintFftScale (juce::Graphics& g) const
{
    // To speed things up we make sure we stay within the graphics context so we can disable clipping at the component level
    ...
    const auto axisColour = juce::Colours::darkgrey.darker();
    const auto textColour = juce::Colours::grey.darker();

    g.setColour (axisColour);
    g.drawRect (getLocalBounds().toFloat());

    g.setFont (juce::Font (GUI_SIZE_I(0.4)));
    ...
    for (auto t = 0; t < numTicks; ++t)
    {
        ...
        const auto dBStr = juce::String (static_cast<int> (toDbVFromPx (scaleY)));
        ...
        g.drawText (dBStr, lblX, lblY, lblW, lblH, juce::Justification::topLeft, false);
    }
    ...
    g.drawFittedText (hertzToString (minFreq, 0, false, false), GUI_SIZE_I(0.1), ty, GUI_BASE_SIZE_I, GUI_SIZE_I(0.5), juce::Justification::topLeft, 1, 1.0f);
	for (auto f : gridFrequencies)
	{
		if (f >= minFreq && f <= maxFreq)
		{
			...
			if (scaleX >= nextThreshX)
			{
                ...
                g.setColour (textColour);
                g.drawFittedText (hertzToString (f, 0, false, false), scaleX + GUI_SIZE_I(0.1), ty, GUI_BASE_SIZE_I, GUI_SIZE_I(0.5), juce::Justification::topLeft, 1, 1.0f);
...
inline float FftScope<Order>::toDbVFromLinear (const float linear) const
{
    if (linear <= 0.0f)
        ...
        return juce::jmax (dbMin, fasterlog2 (linear) * 6.0206f);
}
...
{
    return juce::jmax (1.0f, (dB - dbMax) * yRatio) - 1.0f;
}
...
template <int Order>
juce::String FftScope<Order>::hertzToString (const double frequencyInHz, const int numDecimals, const bool appendHz, const bool includeSpace) const
{
    juce::String space(includeSpace ? " " : "");
    juce::String units;
    juce::String frequency;

    if (frequencyInHz < 1000.0)
    {
        frequency = juce::String (juce::roundToInt<double>(frequencyInHz));
        units = appendHz ? "Hz" : "";
    }
    else if (frequencyInHz < 1000000.0)
    {
        frequency = juce::String (frequencyInHz * 0.001, numDecimals);
        units = appendHz ? "kHz" : "K";
    }
    else
    {
        frequency = juce::String (frequencyInHz * .000001, numDecimals);
...
template <int Order>
juce::Colour FftScope<Order>::getColourForChannel (const int channel)
{
    switch (channel % 6)
    {
        case 0: return juce::Colours::red;
        case 1: return juce::Colours::green;
        case 2: return juce::Colours::blue;
        case 3: return juce::Colours::cyan;
        case 4: return juce::Colours::orange;
        case 5: return juce::Colours::magenta;
        default: return juce::Colours::yellow;
    }
}
...
void FftScope<Order>::preCalculateVariables()
{
    ...
    if (maxFreq == 0.0f)
        ...
        maxFreq = juce::jmin (maxFreq, nyquist);
