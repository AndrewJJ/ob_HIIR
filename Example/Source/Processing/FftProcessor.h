...
public:
    ...
    void setWindowingMethod (juce::dsp::WindowingFunction<float>::WindowingMethod);
...
private:
    ...
    juce::AudioSampleBuffer window;
    juce::AudioSampleBuffer amplitudeEnvelope;
    float amplitudeCorrectionFactor = 0.0f;
    juce::Atomic<bool> amplitudeEnvelopeEnabled = false;
    juce::Atomic<float> amplitudeReleaseConstant = 0.0f;

    juce::OwnedArray <AudioProbe <FftFrame>> freqProbes;
    juce::OwnedArray <AudioProbe <FftFrame>> phaseProbes;
};
                                      ...
                                      size (1 << Order)
{
    ...
    setWindowingMethod(juce::dsp::WindowingFunction<float>::hann);
}

template <int Order>
void FftProcessor<Order>::prepare (const juce::dsp::ProcessSpec& spec)
{
...
void FftProcessor<Order>::performProcessing (const int channel)
{
    ...
    juce::FloatVectorOperations::multiply (temp.getWritePointer (0), window.getWritePointer (0), size);

    // Perform FFT
    fft.performFrequencyOnlyForwardTransform (temp.getWritePointer (0));

    // Correct amplitude
    juce::FloatVectorOperations::multiply (temp.getWritePointer (0), amplitudeCorrectionFactor, size);

    if (amplitudeEnvelopeEnabled.get())
    {
        // Compute envelope on amplitude
        juce::FloatVectorOperations::addWithMultiply (temp.getWritePointer (0), amplitudeEnvelope.getWritePointer (channel), amplitudeReleaseConstant.get(), size);
...
template <int Order>
void FftProcessor<Order>::setWindowingMethod (juce::dsp::WindowingFunction<float>::WindowingMethod)
{
    const auto windowType = juce::dsp::WindowingFunction<float>::hann;
    juce::dsp::WindowingFunction<float>::fillWindowingTables (window.getWritePointer (0), static_cast<size_t> (size), windowType);
